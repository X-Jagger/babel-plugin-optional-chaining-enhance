 ...
 * @modify date 2019-07-17 18:33:38
...
export default declare((api, options) => {
    api.assertVersion(7);
    const { loose = false, auto = true } = options;
    let visitor = {};
    ...
    if (auto) {
        const isValidPath = path => {
            ...
            if (
                !path.node.extra.originalMark ||
                (path.node.extra.originalMark &&
                    path.node.extra.hasBeenTransfromedByBPOC)
            ) {
                return false;
            }
            return true;
        ...
        const addMarkVisitor = {
            // distinguish other plugin's generated  Memberexpression/CallExpression, exclude them
            "MemberExpression|CallExpression": function(path) {
        ...
        visitor = {
            "ExpressionStatement|VariableDeclaration|FunctionDeclaration"(
                ...
                path.traverse(addMarkVisitor); // add mark
            },
            "MemberExpression|CallExpression": function MemberExpressionCallExpression(
                path
            ) {
                ...
                while (isInWhile(optionalPath)) {
                    if (optionalPath.isMemberExpression()) {
                        ...
                        optionalPath.node.extra = Object.assign(
    ...
    // optional chaining visitor
    visitor[
        "OptionalCallExpression|OptionalMemberExpression"
    ] = function OptionalCallExpressionOptionalMemberExpression(path) {
        ...
        for (let i = optionals.length - 1; i >= 0; i--) {
            ...
            if (loose && isCall) {
                ...
                if (ref) {
                    check = t.assignmentExpression(
                        "=",
                        t.cloneNode(ref), // Here `chain` MUST NOT be cloned because it could be updated
            ...
            if (isCall && t.isMemberExpression(chain)) {
                if (loose) {
                    ...
                    if (context) {
                        chain.object = t.assignmentExpression(
                            "=",
                            context,
                            object
                        );
